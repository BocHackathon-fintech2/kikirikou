...
public class EnvironmentImpl implements Environment {
    ...
    private <T> LinkedList<T> stackFor(Class<T> type) {
        ...
        return (LinkedList<T>) typeToStack.computeIfAbsent(type, k -> CollectionFactory.newLinkedList());
    }
        ...
        if (result == null) {
            ...
            throw new UnknownValueException(String.format("No object of type %s is available from the Environment.", type.getName()),
                    new AvailableValues("Environmentals",
                            F.flow(typeToStack.entrySet()).
                                    remove(element -> element.getValue().isEmpty()).
                                    map(element -> element.getKey().getName()).toList()));
        }
